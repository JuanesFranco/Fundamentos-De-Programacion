...
#PARTIDOS EN LOS QUE HA PARTICIPADO LA SELECCION 
def once():
    suma=0
...
#16
#DATOS DE LA SELECCION POR CAMPEONATO ( PARTIDOS GANADOS-PERDIDOS)
...
#17
#MARCADORES DE LA SELECCION GOLES A FAVOR Y EN CONTRA- PARTIDOS PERDIDOS Y GANADOS
def GOLES():
    nomb=df.iloc[:,1]
    nomb2=df.iloc[:,2]
    gol=df.iloc[:,3]
...
GOLES()
18
#MEJOR SELECCION POR TORNEO (LOCAL-VISITANTE) 
def mejor_seleccion():
    nomb=df.iloc[:,1]
    nomb2=df.iloc[:,2]
